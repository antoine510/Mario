...
Menu::Menu(Level** level)
{
    ...
    soundStack->changeMusic(MUSIC_MENU);
}
...
MarioMode Menu::update(KeyStates* keystates)
{
    ...
    if(keyPress && !m_keyPressed)      // Register user action once
    {
        ...
        m_buttons[m_mode][m_selection]->setState(UNSELECTED);
        if(keystates->isDown() && (m_selection < m_buttons[m_mode].size()-1))
        {
            m_selection++;
            soundStack->addToStack(SOUND_MENU_CURSOR);
        }
        ...
        m_buttons[m_mode][m_selection]->setState(SELECTED);
...
{
    soundStack->addToStack(SOUND_MENU_SELECTION);
    switch (m_mode)
    {
        case MAIN_MENU:
            switch (m_selection)

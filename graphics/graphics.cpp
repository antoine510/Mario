...
void Main::renderLevel()
{
    ...
    {
        if(!(*it)->isSet()) {
            (*it)->setMyTex(m_gfxData->textureSetHandler->getTexSet(TEXTYPE_OBJECT, (*it)->getID()));
            (*it)->updateBlitRects();
            sortStack = true;
        }
...
{
    if(!button->isValid()) {
        Texture* temp = m_gfxData->textureSetHandler->getTex(TEXTYPE_BUTTON, button->m_texID, button->m_state);
        button->getRect().setWH(temp->getW(), temp->getH());
        temp->blit(&button->m_rectangle);
    }
    else {
        m_gfxData->textureSetHandler->getTex(TEXTYPE_BUTTON, button->m_texID, button->m_state)->blit(&button->m_rectangle);
    }
}

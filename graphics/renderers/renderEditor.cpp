...
RenderEditor::RenderEditor(GfxData* renderDatas)
{
    ...
    m_groundParams.setMyTex(m_gfxData->textureSetHandler->getTexSet(TEXTYPE_OBJECT, 3));
...
void RenderEditor::render(Editor* editor)
{
    ...
    m_gfxData->textureSetHandler->getDefault(TEXTYPE_OBJECT, 1)->blitCentered(entityRect);
    entityRect.addX(SCREEN_WIDTH/10);
    m_gfxData->textureSetHandler->getDefault(TEXTYPE_OBJECT, 2)->blitCentered(entityRect);
    entityRect.addX(SCREEN_WIDTH/10);
    m_gfxData->textureSetHandler->getDefault(TEXTYPE_OBJECT, 5)->blitCentered(entityRect);
    entityRect.addX(SCREEN_WIDTH/10);
    m_gfxData->textureSetHandler->getDefault(TEXTYPE_OBJECT, 6)->blitCentered(entityRect);

...
HandPattern::HandPattern(const std::string& patternName) {
...
void HandPattern::readHPSections(xmlNodePtr sections) {
	...
	while(sectionN) {
		...
		if(t->vertical) { t->rect.addX(-1); t->rect.addH(-1); } else { t->rect.addY(-1); t->rect.addW(-1); }
...
HandPattern::EventType HandPattern::eventTypeFromString(const std::string& name) {
	if(name == "EnableSection") return EventType::ENABLE_SECTION;
	if(name == "DisableSection") return EventType::DISABLE_SECTION;
	if(name == "Wait") return EventType::WAIT;
	if(name == "MoveHand") return EventType::MOVE_HAND;
...
void HandPattern::initEvent(HandPatternQueue& eventQueue, const HandPatternEvent* event) {
	if(event->type == EventType::ENABLE_SECTION) {
	...
	} else if(event->type == EventType::DISABLE_SECTION) {
		int id = event->sectionID;
		eventQueue.hpHand->updatePos(!m_sections[id].vertical, m_sections[id].rect, 0);
	} else if(event->type == EventType::MOVE_HAND) {

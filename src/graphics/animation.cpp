...
Animation::Animation(SDL_Renderer* renderer, std::string path, int frameWidth, int frameCount, int delay) :
	...
	for(int i = 0; i < maxUsers; i++) m_timer[i].setDelay(delay);
}
...
NLAnimation::NLAnimation(SDL_Renderer* renderer, std::string path, int frameWidth, int frameCount, int delay, std::string fallbackPath) : Animation(renderer, path, frameWidth, frameCount, delay),

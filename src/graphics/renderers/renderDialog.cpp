...
void RenderDialog::render()
{
    if(dialog != NULL)
    {
        ...
        {
            Texture* temp = m_gfxData->textureSetHandler->getDefault(TextureType::OTHER, dialog->getTexID());    //Update dialogBox WH to compute text pos
        ...
        m_gfxData->textureSetHandler->getDefault(TextureType::OTHER, dialog->getTexID())->blit(&m_dialogBoxPos);

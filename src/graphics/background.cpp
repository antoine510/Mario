...
Background::Background(int texID, int spanWidth, int spanSpeed) :
	...
	m_drawingPos(0, 0, 0, 0, FRAME_SCREEN)
{}
...
RenderBackground::RenderBackground(const GfxData& gfxData) :
	m_gfxData(gfxData)
{
...
{
    m_bgs[id].refresh();
	m_gfxData.textureSetHandler->getDefault(TextureType::BACKGROUND, m_bgs[id].getTexID())->blit(m_bgs[id].getRect());
}
